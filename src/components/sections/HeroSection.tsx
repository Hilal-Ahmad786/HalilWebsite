        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="grid grid-cols-2 gap-6">
            {siteConfig.stats.map((stat, index) => (
              <div
                key={index}
                className={`bg-white/10 backdrop-blur-sm p-8 border-l-4 border-${stat.color}-400 hover:bg-white/15 transition-all transform hover:-translate-y-2`}
                style={{
                  animationDelay: `${index * 0.1}s`,
                }}
              >
                <div className={`text-5xl md:text-6xl font-black text-${stat.color}-400 mb-3 leading-none`}>
                  {stat.value}
                </div>
                <div className="text-sm text-gray-300 font-bold tracking-wider">
                  {stat.label}
                </div>
              </div>
            ))}
