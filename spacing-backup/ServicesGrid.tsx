    ...
    <section className="py-section-y bg-white">
      <div className="container mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-md">
          {siteConfig.services.map((service, index) => {
            ...
            return (
              <div
                key={service.id}
                className={`group bg-white p-card border-l-4 ${colors.border} shadow-md hover:shadow-2xl transition-all duration-300 hover:transform hover:-translate-y-2`}
              >
                {/* Icon */}
                <div className={`text-6xl mb-md transform group-hover:scale-110 transition-transform duration-300`}>
                  {service.icon}
                </div>

                {/* Title */}
                <h3 className="text-2xl font-black text-gray-900 mb-sm leading-tight">
                  {service.title}
                </h3>

                {/* Description */}
                <p className="text-gray-600 mb-md font-medium">
                  {service.shortDesc}
                </p>
                ...
                <Link
                  ...
                  className={`block w-full ${colors.button} py-4 font-black text-center transition-all duration-300 transform group-hover:scale-105`}
                >
                  DETAY & TEKLİF AL
                </Link>

                {/* Hover Accent */}
                <div className={`mt-md pt-4 border-t ${colors.border} opacity-0 group-hover:opacity-100 transition-opacity`}>
        ...
        <div className="mt-xl text-center">
          <p className="text-xl text-gray-700 mb-md font-bold">
            Aracınız hangi kategoride? Hemen arayın, anında teklif alın!
          </p>
          <a
            href={`tel:${siteConfig.phone}`}
            className="inline-flex items-center gap-sm bg-lime-400 text-gray-900 px-button-x py-button-y text-2xl font-black hover:bg-lime-300 transition-all transform hover:scale-105 shadow-xl"
          >
